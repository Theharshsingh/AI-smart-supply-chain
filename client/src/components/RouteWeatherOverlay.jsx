import { Polyline, CircleMarker, Popup } from 'react-leaflet';
import { RISK } from '../services/riskEngine';

/**
 * Color map per spec:
 *  Green     = Safe (score 0)
 *  Yellow    = Mild Risk (1-24)
 *  Orange    = Moderate (25-54)
 *  Red       = High Risk (55-69)
 *  Dark Red  = Severe / Thunderstorm (70+)
 */
function riskScoreToColor(numericScore) {
  if (numericScore >= 70) return '#7f1d1d'; // dark red — severe/thunderstorm
  if (numericScore >= 55) return '#ef4444'; // red — high
  if (numericScore >= 25) return '#f59e0b'; // orange — moderate
  if (numericScore >= 1)  return '#eab308'; // yellow — mild
  return '#22c55e';                          // green — safe
}

function getNumericScore(wp) {
  return wp.riskInfo?.numericScore ?? 0;
}

export default function RouteWeatherOverlay({ weatherPoints }) {
  if (!weatherPoints?.length) return null;

  return (
    <>
      {/* Coloured polyline segments */}
      {weatherPoints.map((wp, i) => {
        if (i === 0) return null;
        const prev   = weatherPoints[i - 1];
        const score  = getNumericScore(wp);
        const color  = riskScoreToColor(score);
        const isBad  = score >= 25;

        return (
          <Polyline
            key={`wx-seg-${i}`}
            positions={[[prev.lat, prev.lng], [wp.lat, wp.lng]]}
            pathOptions={{
              color,
              weight:     isBad ? 8 : 5,
              opacity:    isBad ? 0.9 : 0.55,
              dashArray:  score >= 70 ? '5 3' : score >= 55 ? '8 4' : null,
              lineCap:    'round',
              lineJoin:   'round',
            }}
          />
        );
      })}

      {/* Weather markers at each sampled point */}
      {weatherPoints.map((wp, i) => {
        const wx    = wp.weather;
        if (!wx) return null;
        const score  = getNumericScore(wp);
        const color  = riskScoreToColor(score);
        const risk   = wp.riskInfo;
        const radius = score >= 70 ? 12 : score >= 55 ? 10 : score >= 25 ? 8 : score >= 1 ? 6 : 4;

        const etaDisplay = wp.etaFormatted || (wp.etaMs
          ? new Date(Date.now() + wp.etaMs).toLocaleTimeString([], { hour: 'numeric', minute: '2-digit', hour12: true })
          : 'Now');

        const speedRedPct = Math.round((risk?.speedReduction || 0) * 100);

        return (
          <CircleMarker
            key={`wx-pt-${i}`}
            center={[wp.lat, wp.lng]}
            radius={radius}
            pathOptions={{ color, fillColor: color, fillOpacity: 0.92, weight: 2 }}
          >
            <Popup>
              <div style={{
                background: '#111827', color: '#e2e8f0',
                padding: '10px 12px', borderRadius: 10,
                minWidth: 210, fontFamily: 'Inter, sans-serif',
              }}>
                {/* Header */}
                <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 8 }}>
                  <span style={{ fontWeight: 700, fontSize: 13, flex: 1 }}>{wx.condition}</span>
                  <span style={{
                    fontSize: 9, fontWeight: 800, padding: '2px 7px', borderRadius: 4,
                    background: color + '33', color, textTransform: 'uppercase', letterSpacing: '0.06em',
                  }}>
                    {risk?.label || 'Safe'} {score > 0 ? `(${score})` : ''}
                  </span>
                </div>

                {/* Risk reasons */}
                {risk?.reasons?.length > 0 && (
                  <div style={{ marginBottom: 7, display: 'flex', flexDirection: 'column', gap: 2 }}>
                    {risk.reasons.map((r, ri) => (
                      <div key={ri} style={{ fontSize: 10, color, fontWeight: 600 }}>{r}</div>
                    ))}
                  </div>
                )}

                {/* Details */}
                <div style={{ display: 'flex', flexDirection: 'column', gap: 3, fontSize: 11 }}>
                  <div style={{ color: '#94a3b8', fontStyle: 'italic', marginBottom: 2 }}>{wx.description}</div>
                  <div>🌡️ <b style={{ color: '#f1f5f9' }}>{wx.temp}°C</b> · 💧 {wx.humidity}%</div>
                  <div>💨 Wind: <b style={{ color: wx.windSpeed > 60 ? '#ef4444' : wx.windSpeed > 40 ? '#f59e0b' : '#f1f5f9' }}>{wx.windSpeed} km/h</b>
                    {wx.windGusts > 0 && <span style={{ color: '#a78bfa' }}> (gusts {wx.windGusts} km/h)</span>}
                  </div>
                  {wx.precipitation > 0 && <div>🌧️ Precip: <b>{wx.precipitation} mm</b></div>}
                  {wx.precipProb > 0  && <div>🌂 Rain prob: <b>{wx.precipProb}%</b></div>}
                  {wx.visibility != null && (
                    <div>👁️ Visibility: <b style={{ color: wx.visibility < 2 ? '#ef4444' : wx.visibility < 5 ? '#f59e0b' : '#f1f5f9' }}>{wx.visibility} km</b></div>
                  )}
                  {speedRedPct > 0 && (
                    <div style={{ color: '#f59e0b', marginTop: 2 }}>🚗 Speed reduction: <b>↓{speedRedPct}%</b></div>
                  )}
                  <div style={{ color: '#60a5fa', marginTop: 4 }}>
                    ⏱ ETA: <b>{etaDisplay}</b>
                    {wx.forecastTime && <span style={{ color: '#475569', fontSize: 9 }}> (forecast)</span>}
                  </div>
                  <div style={{ color: '#475569' }}>
                    📍 {wp.distFromStartKm === 0 ? 'Route start' : `${wp.distFromStartKm} km from start`}
                  </div>
                </div>
              </div>
            </Popup>
          </CircleMarker>
        );
      })}
    </>
  );
}
